...
import { ClassRecord, Tutor, Student, calculatePaymentAmount } from './models.js';
  ...
  try {
    ...
    if (lessonDate !== submitDate) {
      return res.status(400).json({
        message: 'Class records must be submitted the same day as the lesson. Contact Admin for approval if late.'
      });
    }
...
router.get('/', async (req, res) => {
  ...
  const records = await ClassRecord.find(filter).populate('tutorId').populate('studentId').sort({ dateSubmitted: -1 });
  res.json(records);
});
